    ...
    const paint = (prog: number) => {
      ...
      for (let y = 0; y < rows; y++) {
        ...
        for (let x = 0; x < cols; x++) {
          ...
          // Use BAYER8 for gradient, BAYER for others
          ...
          const bias = variant === "dotted" ? 0.35 : 0
          ...
          if (variant === "gradient") {
            ...
            lit =
              variant === "solid" ||
              density > bayerMatrix[bayerY][bayerX] - 0.1 * it - bias
          }
          ...
          if (variant === "gradient") {
            // Gradient fade: 15% at inner radius → 100% at outer radius
            alphaMultiplier = 0.15 + density * 0.85
          }
          const k = (0.35 + density * 0.65) * (1 + 0.22 * it)
          const alpha = Math.min(1, (lit ? k : k * OFF_TIER) * selDim * alphaMultiplier)
