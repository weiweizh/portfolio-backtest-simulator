  ...
  return (
    <AnimatePresence>
      {show && items.length > 0 && (
        ...
        >
          {heading && (
            <div className="mb-0.5 font-mono text-[16px] text-popover-foreground/75">
              {heading}
            </div>
          )}
          <div className="flex flex-col gap-0.5">
            {items.map((item) => (
              <div
                key={item.name}
                className="flex items-center gap-1.5 font-mono text-[17px] text-popover-foreground tabular-nums"
              ...
              >
                ...
                />
                <span className="text-popover-foreground/85">{item.label}</span>
                <span className="ml-auto pl-2 text-popover-foreground">
                  {valueFormatter
